...
namespace nonius {
    ...
    private:
        ...
        void do_measurement_start(execution_plan<fp_seconds> plan) override {
            ...
            if(!summary) stream() << "collecting " << n_samples << " samples, " << plan.iterations_per_sample << " iterations each, in estimated " << detail::pretty_duration(plan.estimated_duration) << "\n";
        }
        ...
        void do_analysis_complete(sample_analysis<fp_seconds> const& analysis) override {
            ...
            if(verbose) stream() << "variance introduced by outliers: " << detail::percentage(analysis.outlier_variance) << "\n";
            const char* effect;
        ...
        void print_environment_estimate(environment_estimate<fp_seconds> e, int iterations) {
            stream() << std::setprecision(7);
            stream().unsetf(std::ios::floatfield);
            stream() << "mean is " << detail::pretty_duration(e.mean) << " (" << iterations << " iterations)\n";
            if(verbose) print_outliers(e.outliers);
        }
        void print_outlier_count(const char* description, int count, int total) {
            if(count > 0) stream() << "  " << count << " (" << detail::percentage_ratio(count, total) << ") " << description << "\n";
        }
        void print_outliers(outlier_classification o) {
            stream() << "found " << o.total() << " outliers among " << o.samples_seen << " samples (" << detail::percentage_ratio(o.total(), o.samples_seen) << ")\n";
            if(verbose) {
        ...
        void print_statistic_estimate(const char* name, estimate<fp_seconds> estimate) {
            stream() << std::setprecision(7);
            stream().unsetf(std::ios::floatfield);
            stream() << name << ": " << detail::pretty_duration(estimate.point);
            if(!summary) {
                stream() << ", lb " << detail::pretty_duration(estimate.lower_bound)
